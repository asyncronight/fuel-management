...
public class UserCalculService {
    ...
    public List<ChantierBatch> getListChantierWithQuantities(int month, int year) {
        ...
        for (Map.Entry<Chantier, Long> entry : sum.entrySet()) {
            chantierBatch = new ChantierBatch(month, year, entry.getValue(), sum2.get(entry.getKey()), chargeLocataire.get(entry.getKey()), chargeLocataireExterne.get(entry.getKey()), entry.getKey());
