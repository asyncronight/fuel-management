...
public interface BonEnginRepo extends JpaRepository<BonEngin, Long> {
    ...
    @Query(nativeQuery = true, value = "select * from bon_engin where engin_id=?1 and compteur_hen_panne=false and plein=true order by date_bon,compteur_absoluh desc limit 1")
    BonEngin findLastBonEnginH_toConsommation(Long id);

    @Query(nativeQuery = true, value = "select * from bon_engin where engin_id=?1 and compteur_kmen_panne=false and plein=true order by date_bon,compteur_absolu_km desc limit 1")
