...
public class BatchFaker {
    ...
    public void insertBatchChantier() {
        ...
        for (Chantier c : chantierService.getList()) {
            for (int i = 1; i <= 6; i++) {
                chantierBatch = new ChantierBatch(i, 2018, 1050 + i * 2 + 1L, 453 * i / 2 + 1L, 20 + i * 2 + 1L, 11 * i / 2 + 1L, c);
                chantierBatchRepo.save(chantierBatch);
            }
            for (int i = 5; i <= 12; i++) {
                chantierBatch = new ChantierBatch(i, 2017, 1050 + i * 2 + 1L, 453 * i / 2 + 1L, 20 + i * 2 + 1L, 11 * i / 2 + 1L, c);
                chantierBatchRepo.save(chantierBatch);
            }
        }
